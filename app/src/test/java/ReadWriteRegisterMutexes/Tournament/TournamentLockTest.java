...
public class TournamentLockTest {
    ...
    /** This is here to test the test for the TournamentLock is valid */
    @Test
    public void testReentrantLockIncrement() {
        int numWorkers = 8;
        ...
        for (int i=0; i< numWorkers; i++) {
            // Even workers add, odd workers substract
            workers[i] = new WorkerThreadR((((i%2) == 0) ? true : false), i);
        }

        WorkerThreadR.c = 0;
        WorkerThreadR.increments = 100000;
        WorkerThreadR.lock = lock;
        ...
        // Check we got the right result
        assertEquals("Synchronization error: ", 0, WorkerThreadR.c);
        System.out.println("Success: c = " + WorkerThreadR.c + " expected 0");
    }
}
...
class WorkerThreadR implements Runnable {
    public volatile static int c;
    public volatile static int increments;
    public volatile static ReentrantLock lock;
    public boolean add;
    public int tid;

    public WorkerThreadR(boolean add, int tid) {
        this.add = add;
        this.tid = tid;
    }
