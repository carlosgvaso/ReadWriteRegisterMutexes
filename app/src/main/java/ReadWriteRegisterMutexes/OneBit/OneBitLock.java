...
public class OneBitLock implements Lock {
    ...
    public void lock(int pid) {
        ...
        while(b[pid] == false) {
            b[pid] = true; // Process i indicates that its interested in critical section
            int j = 0;
            while ((b[pid] == true) && j < pid) { // Check the bits of all the processes that are less than its process id
                if (b[j] == true) {
                    b[pid] = false;  // Set to false so that the outer do - while starts again
                    while (b[j] == true) { // If some other process j's bit is true then wait
                        ; //no-op
        ...
        for(int j=pid+1; j<this.N; j++){ // Check the bits of all processes that are higher than current process id
            while(b[j] == true){  // Wait till the other process's bit is false
                ;//wait no-op
